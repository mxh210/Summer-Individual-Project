...
Its best cross-validation AUPRC was 0.1515. Although the interaction specification achieved a slightly higher cross-validation AUPRC, it produced a worse operating point on the separate validation set at the required recall level. The additive model was therefore retained. Its validation-selected threshold was 0.0674.
...
\midrule
Reg. Logistic Regression & 0.0674 & 0.1499 & 0.6374 & 0.8107 & 0.1108 & 0.3581 & 0.6419 & 0.3582 & 0.6570 & 0.0800 \\
...
\midrule
Reg. Logistic Regression & 0.1498 & 0.1499 & 0.5887 & 0.8107 \\
...
Regularised Logistic Regression and Random Forest produced the lowest Brier scores, both approximately 0.0800. FT-Transformer and CatBoost followed at 0.0852 and 0.0857. XGBoost and Decision Tree produced substantially higher Brier scores of 0.1548 and 0.2357.

These results indicate that the raw probability outputs from Logistic Regression and Random Forest are currently more promising as absolute risk estimates. However, the Brier score reflects both calibration and discrimination and is not sufficient by itself to establish good calibration. A full calibration assessment should therefore add calibration curves and, where possible, calibration slope and intercept before predicted probabilities are interpreted as clinical risks.

The result is also relevant to class weighting. Logistic Regression and Random Forest selected no additional positive-class weighting, whereas Decision Tree used balanced class weights, XGBoost selected \texttt{scale\_pos\_weight = 6}, and both CatBoost and FT-Transformer selected a positive-class weight of 2. Weighting can improve emphasis on the minority class while shifting the scale of predicted probabilities, so calibration and class-weight sensitivity should be considered together.
...
The next planned experiment is the class-weight sensitivity analysis requested after the main report, particularly for Regularised Logistic Regression and Random Forest because their hyperparameter searches selected no additional weight for the readmission class. This analysis should test whether deliberately increasing positive-class weight can reduce the false-positive cost required to maintain high recall, or whether it mainly shifts the probability scale without improving underlying discrimination.

After class-weight sensitivity has been assessed, final model selection should consider three aspects together: threshold-independent discrimination (AUPRC and AUROC), behaviour at the clinically motivated operating point (recall, precision, specificity/FPR, F2, and proportion flagged), and calibration. Interpretability and subgroup performance can then be used as additional considerations before making any claim about practical healthcare use.
